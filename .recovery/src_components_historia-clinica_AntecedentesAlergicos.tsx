...
const AntecedentesAlergicos: React.FC<AntecedentesAlergicosProps> = ({
  ...
  const generateRedaccion = () => {
    ...
    if (formData.antecedentesAlergicos.tiposAlergias?.ambiente) alergias.push("factores ambientales");

    if (alergias.length > 0) {
      mainText += `Paciente refiere antecedentes alérgicos a ${alergias.join(", y ")}`;
      if (formData.antecedentesAlergicos.cualesAlergias) {
        mainText += ` (${formData.antecedentesAlergicos.cualesAlergias})`;
      }
      mainText += ". ";
      if (formData.antecedentesAlergicos.especificacionAlergias) {
        mainText += `Las reacciones se manifiestan con ${formData.antecedentesAlergicos.especificacionAlergias}. `;
      }
    } else {
      mainText += "Niega alergias a medicamentos, alimentos o factores ambientales. ";
    }

    if (formData.antecedentesAlergicos.administradoAnestesia) {
      mainText += "Refiere administración previa de anestesia";
      if (formData.antecedentesAlergicos.tipoAnestesia) {
        mainText += ` (${formData.antecedentesAlergicos.tipoAnestesia})`;
      }
      if (formData.antecedentesAlergicos.reaccionAnestesia) {
        mainText += " habiendo presentado una reacción adversa";
        if (formData.antecedentesAlergicos.descripcionReaccion) {
          mainText += ` descrita como ${formData.antecedentesAlergicos.descripcionReaccion}. `;
        } else {
          mainText += ". ";
        }
      } else {
        mainText += " sin complicaciones ni reacciones adversas asociadas. ";
      }
    } else {
      mainText += "Niega administración previa de anestesia local o general. ";
    }
    ...
    if (formData.antecedentesAlergicos.adicciones?.drogas) adicciones.push("sustancias ilícitas");

    if (adicciones.length > 0) {
      mainText += `Presenta adicciones activas a ${adicciones.join(", ")}`;
      if (formData.antecedentesAlergicos.detallesAdicciones) {
        mainText += `, especificando: ${formData.antecedentesAlergicos.detallesAdicciones}.`;
      } else {
        mainText += ".";
      }
    } else {
      mainText += "Niega tabaquismo, alcoholismo u otras toxicomanías.";
    }

    content += `<div style="text-align: justify;">${mainText}</div>`;
