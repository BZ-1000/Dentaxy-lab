...
import {
  Users, ArrowRight, Sparkles, MessageCircle, Heart,
  TrendingUp, Shield, Star, Lock, X, Mail, User, Loader2, CheckCircle,
} from "lucide-react";
...
export default function ClubPage() {
  ...
  const [waitlistCount] = useState(1492);

  return (
    <div className="club-theme min-h-screen w-full relative bg-white overflow-hidden font-sans">
      <WaitlistMasterModal isOpen={waitlistOpen} onClose={() => setWaitlistOpen(false)} preselectedModule="Club" />
      <OrganicShopFrame
      ...
      <main className="relative z-10 min-h-screen w-full flex flex-col items-center justify-center p-6 pt-24">
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-medium text-neutral-600">
                <span className="text-neutral-900 font-bold">{waitlistCount.toLocaleString()}</span> en espera
              </p>
            </div>
          </motion.div>
          ...
          <motion.h1 variants={fadeUp} className="text-5xl md:text-7xl lg:text-8xl font-bold tracking-tight">
            ...
            <span className="eco-gradient-text">Club</span>
          </motion.h1>

          <motion.p variants={fadeUp} className="text-xl md:text-2xl eco-accent-text font-medium">
            La comunidad del gremio dental mexicano
          </motion.p>

          <motion.p variants={fadeUp} className="text-lg text-gray-500 max-w-2xl mx-auto">
            Debate casos clínicos, haz networking real con colegas, participa en retos y crece con el gremio odontológico en tiempo real.
          </motion.p>
          ...
          <motion.div variants={staggerContainer} className="flex flex-col sm:flex-row gap-6 w-full max-w-2xl">
            <motion.button
              ...
              className="flex-1 group relative bg-white border border-neutral-100 p-6 rounded-3xl text-left shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_-12px_rgba(249,115,22,0.2)] transition-all duration-500 hover:-translate-y-1"
            >
              <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
                <ArrowRight className="w-4 h-4 text-orange-500 -rotate-45" />
              </div>
              <div className="bg-orange-50 w-14 h-14 rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-500">
                <Mail className="w-7 h-7 text-orange-600" />
              </div>
              <h3 className="font-bold text-neutral-900 text-xl group-hover:text-orange-700 transition-colors">Lista de Espera</h3>
              <p className="text-sm text-neutral-500">Unirme a la comunidad</p>
            </motion.button>

            <motion.button
              variants={scaleIn}
              onClick={() => navigate("/hub")}
              className="flex-1 group relative bg-white border border-neutral-100 p-6 rounded-3xl text-left shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_-12px_rgba(249,115,22,0.1)] transition-all duration-500 hover:-translate-y-1"
            >
          ...
          >
            {[
              { icon: MessageCircle, label: "Casos Clínicos en Vivo", color: "text-orange-500", bg: "bg-orange-50" },
              { icon: TrendingUp, label: "Networking Gremial", color: "text-amber-500", bg: "bg-amber-50" },
              { icon: Star, label: "Rankings y Retos", color: "text-yellow-500", bg: "bg-yellow-50" },
            ].map((f, i) => (
          ...
          >
            ...
            {openModal === "admin" && (
              <div className="space-y-4 text-center">
                ...
                <h2 className="text-2xl font-bold">Admin Club</h2>
