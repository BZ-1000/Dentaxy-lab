...
import {
  Newspaper, ArrowRight, Sparkles, Rss, BookOpen,
  Globe, TrendingUp, Lock, X, Mail, User, Loader2, CheckCircle, Filter,
} from "lucide-react";
...
export default function NewsPage() {
  ...
  const [waitlistCount] = useState(657);

  return (
    <div className="news-theme min-h-screen w-full relative bg-white overflow-hidden font-sans">
      <WaitlistMasterModal isOpen={waitlistOpen} onClose={() => setWaitlistOpen(false)} preselectedModule="News" />
      <OrganicShopFrame
      ...
      <main className="relative z-10 min-h-screen w-full flex flex-col items-center justify-center p-6 pt-24">
        ...
        >
          ...
          <motion.h1 variants={fadeUp} className="text-5xl md:text-7xl lg:text-8xl font-bold tracking-tight">
            ...
            <span className="eco-gradient-text">News</span>
          </motion.h1>

          <motion.p variants={fadeUp} className="text-xl md:text-2xl eco-accent-text font-medium">
            El pulso de la odontología moderna
          </motion.p>

          <motion.p variants={fadeUp} className="text-lg text-gray-500 max-w-2xl mx-auto">
            Noticias, tendencias globales y actualizaciones científicas filtradas con inteligencia. Solo lo relevante para el profesional dental moderno.
          </motion.p>
          ...
          <motion.div variants={staggerContainer} className="flex flex-col sm:flex-row gap-6 w-full max-w-2xl">
            <motion.button variants={scaleIn} onClick={() => setWaitlistOpen(true)}
              className="flex-1 group relative bg-white border border-neutral-100 p-6 rounded-3xl text-left shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_-12px_rgba(14,165,233,0.2)] transition-all duration-500 hover:-translate-y-1"
            >
              <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
                <ArrowRight className="w-4 h-4 text-sky-500 -rotate-45" />
              </div>
              <div className="bg-sky-50 w-14 h-14 rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-500">
                <Rss className="w-7 h-7 text-sky-600" />
              </div>
              <h3 className="font-bold text-neutral-900 text-xl group-hover:text-sky-700 transition-colors">Lista de Espera</h3>
              <p className="text-sm text-neutral-500">Recibir novedades exclusivas</p>
            </motion.button>

            <motion.button variants={scaleIn} onClick={() => navigate("/hub")}
              className="flex-1 group relative bg-white border border-neutral-100 p-6 rounded-3xl text-left shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_-12px_rgba(14,165,233,0.1)] transition-all duration-500 hover:-translate-y-1"
            >
          ...
          >
            {[
              { icon: Filter, label: "Filtrado por IA", color: "text-sky-500", bg: "bg-sky-50" },
              { icon: Globe, label: "Fuentes Globales", color: "text-cyan-500", bg: "bg-cyan-50" },
              { icon: BookOpen, label: "Artículos Científicos", color: "text-blue-500", bg: "bg-blue-50" },
            ].map((f, i) => (
          ...
          >
            ...
            {openModal === "admin" && (
              <div className="space-y-4 text-center">
                ...
                <h2 className="text-2xl font-bold">Admin News</h2>
