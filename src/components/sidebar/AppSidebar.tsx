...
export const AppSidebar = ({ className }: AppSidebarProps) => {
  ...
  const DesktopSidebar = () => (
    <motion.aside
      ...
      className={cn(
        'hidden md:flex flex-col h-screen bg-background border-r border-border/50 fixed left-0 top-0 z-[100]',
        className
      )}
    >
      ...
      {mobileOpen && (
        <>
          ...
          <motion.aside
            ...
            exit={{ x: '-100%' }}
            transition={{ type: 'spring', stiffness: 300, damping: 30 }}
            className="fixed left-0 top-0 h-screen w-[220px] bg-background border-r border-border/50 z-[100] md:hidden flex flex-col"
          >
