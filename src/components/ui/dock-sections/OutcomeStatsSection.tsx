...
// --- Card separada: Lenguajes de programación utilizados ---
export const TechnologyUsageCard = () => {
  const cardVariants = { hidden: { opacity: 0, y: 12 }, visible: { opacity: 1, y: 0 } };

  const getChartVar = (index: number) => {
    const vars = ['--chart-1', '--chart-2', '--chart-3', '--chart-4', '--chart-5', '--primary'];
          ...
          {programmingLanguages.map((lang, idx) => (
            <div key={lang.label} className="space-y-1.5">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-foreground/90">{lang.label}</span>
                <span className="text-sm font-semibold text-muted-foreground">{lang.percentage}%</span>
              </div>
              <div className="w-full bg-muted rounded-full h-2 overflow-hidden">
                <motion.div
                  className="h-2 rounded-full"
                  style={{ backgroundColor: `hsl(var(${getChartVar(idx)}))` }}
                  initial={{ width: '0%' }}
                  animate={{ width: `${lang.percentage}%` }}
...
// --- Card separada: Opinión de la Comunidad ---
export const CommunityOpinionCard = () => {
  const { toast } = useToast();
  ...
  const handleRating = async (rating: number) => {
    ...
    try {
      const { error } = await supabase.from('user_ratings').upsert({ user_id: user.id, rating });
      if (error) throw error;
      setUserRating(rating);
      toast({
        title: '¡Gracias por tu opinión!',
    ...
    <Card className="bg-card border border-border/50 rounded-2xl shadow-sm">
      ...
      <CardContent className="pt-0">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Bloque izquierdo: promedio + estrellas interactivas */}
          <div className="flex flex-col items-center text-center gap-2">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col items-start">
                <div className="flex">
                  {[1,2,3,4,5].map((s) => (
                    <Star key={s} className={s <= averageRating ? 'h-4 w-4 text-primary fill-primary' : 'h-4 w-4 text-muted-foreground/40'} />
                  ))}
                </div>
            ...
            <div className="flex gap-1 mt-1">
              {[1,2,3,4,5].map((s) => (
                ...
                >
                  <Star className={(s <= (hoveredStar || userRating)) ? 'h-5 w-5 text-primary fill-primary' : 'h-5 w-5 text-muted-foreground/40'} />
                </motion.button>
              ))}
            </div>
          </div>

          {/* Bloque derecho: distribución */}
          <div className="space-y-2">
            <h4 className="text-sm font-medium text-foreground/90">Distribución</h4>
            {ratingDistribution.map((count, index) => (
              <div key={index} className="flex items-center gap-2 text-xs">
                <span className="font-medium text-muted-foreground">{5 - index}</span>
