...
import { Loader2, X, Save, User, FileText, BookOpen } from "lucide-react";
import { useHistoriaClinica } from '@/hooks/useHistoriaClinica';
import FormulariosSidebar from './historia-clinica/FormulariosSidebar';
import MobileFormulariosSidebarButton from './historia-clinica/MobileFormulariosSidebarButton';
import MobileFormulariosSidebar from './historia-clinica/MobileFormulariosSidebar';
...
const HistoriaClinica = () => {
  ...
  useEffect(() => {
    const mediaQuery = window.matchMedia("(max-width: 640px)"); // Consider mobile < 640px width
  ...
  return (
    <div className={`${theme} min-h-screen w-full flex flex-col sm:flex-row`}>
      {/* On desktop (sm and above), render permanent sidebar */}
      {!isMobile && (
        <FormulariosSidebar 
      ...
      {/* For Mobile: Render floating button and sliding sidebar */}
      {isMobile && (
        <>
          <MobileFormulariosSidebarButton
            isOpen={mobileSidebarOpen}
            onToggle={() => setMobileSidebarOpen(!mobileSidebarOpen)}
          />
          <MobileFormulariosSidebar
            isOpen={mobileSidebarOpen}
      ...
      <div
        ...
        } flex-1 py-12 px-4 sm:px-6 lg:px-8 overflow-auto
          ${isMobile ? "max-w-full px-3 py-6" : ""}
      ...
      >
        <div className="max-w-5xl mx-auto space-y-8">
          <div className="text-center">
            ...
            {/* Patient name input - scaled smaller on mobile */}
            <div
              id="patient-name-input"
              className="max-w-lg mx-auto mb-2 sticky top-4 z-30 backdrop-blur-sm shadow-sm border border-gray-200 p-4 py-[5px] px-[20px] rounded-2xl bg-slate-50
              ...
              <div className="flex items-center gap-3">
                <div className="relative flex-1">
                  ...
                  <Input
                    ...
                    className="pl-8 border-0 bg-transparent focus:ring-0 focus-visible:ring-0 focus-visible:ring-offset-0
                    text-xs sm:text-sm
                    "
                  />
                </div>
                ...
                >
                  ...
                  <span className="hidden sm:inline-block font-medium">Guardar</span>
                </Button>
              </div>
            </div>

            {/* Paciente actual display with smaller text and icon on mobile */}
            {pacienteActual && (
              <div className="flex items-center justify-center gap-2 mb-6">
                <div className="text-xs sm:text-sm text-blue-500 dark:text-blue-400 font-medium truncate max-w-xs">
            ...
            {/* Gender selector - smaller buttons on mobile */}
            <div className="flex items-center justify-center mb-6 gap-3 text-xs sm:text-sm px-2">
          ...
          {/* All form sections - make font smaller on mobile */}
