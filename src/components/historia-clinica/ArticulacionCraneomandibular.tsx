...
const ArticulacionCraneomandibular = ({ formData, handleInputChange }: ArticulacionCraneomandibularProps) => {
  ...
  const generarRedaccionIA = () => {
    const articulacionCraneomandibular = formData.articulacionCraneomandibular || "";
    
    let textoGenerado = "";
    if (!articulacionCraneomandibular || articulacionCraneomandibular.trim() === "") {
      textoGenerado = "Articulación craneomandibular sin alteraciones. Apertura bucal dentro de límites normales, sin chasquidos, crepitaciones o dolor a la palpación.";
    } else {
      textoGenerado = `Articulación craneomandibular: ${articulacionCraneomandibular}`;
    }
  ...
  const limpiarFormulario = () => {
    handleInputChange("articulacionCraneomandibular", "");
    setRedaccionIA("");
            ...
            {showRedaccion ? (
              ...
              <div className="p-6 space-y-6">
                <div className="space-y-2">
                  <Label htmlFor="articulacionCraneomandibular">Describa la articulación craneomandibular:</Label>
                  <Textarea
                    id="articulacionCraneomandibular"
                    placeholder="Apertura bucal, ruidos articulares, dolor, limitaciones de movimiento, etc..."
                    value={formData.articulacionCraneomandibular || ""}
                    onChange={(e) => handleInputChange("articulacionCraneomandibular", e.target.value)}
                  ...
                  <div className="mt-2">
                    <VoiceInput
                      onTranscriptionComplete={(text) => {
                        const currentText = formData.articulacionCraneomandibular || "";
                        handleInputChange("articulacionCraneomandibular", currentText ? `${currentText} ${text}` : text);
                      }}
                    />
                  </div>
                </div>
