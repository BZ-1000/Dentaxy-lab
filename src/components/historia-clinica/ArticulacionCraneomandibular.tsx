...
interface ArticulacionCraneomandibularProps {
  ...
  handleArticulacionCraneomandibularChange: (part: string, value: string | boolean | null) => void;
}
      ...
      <Card className={`bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm shadow-lg rounded-xl border-0 ${isMaximized ? "h-[calc(100vh-2rem)] overflow-y-auto" : ""} ${isMinimized ? "h-16 overflow-hidden" : ""}`}>
        ...
        <div className="flex justify-start px-6 py-2">
          <h2 className="text-xl font-semibold flex items-center gap-2">
            <span className="text-gray-400">XII.</span> ARTICULACIÓN TEMPOROMANDIBULAR
          </h2>
        </div>

        {!isMinimized && (
          <div className="p-6">
            {/* Aquí va el contenido específico del componente */}
            <p>Contenido de Articulación Craneomandibular</p>
          </div>
        )}
      </Card>
    </div>
