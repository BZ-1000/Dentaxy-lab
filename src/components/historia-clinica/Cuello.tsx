...
interface CuelloProps {
  formData: FormDataState;
  handleInputChange: (field: string, value: string) => void;
}

const Cuello = ({ formData, handleInputChange }: CuelloProps) => {
  ...
  const generarRedaccionIA = () => {
    const cuello = formData.cuello || "";
    
    let textoGenerado = "";
    if (!cuello || cuello.trim() === "") {
      textoGenerado = "Cuello cilíndrico, sin adenomegalias, masas o cicatrices visibles. Movilidad conservada y sin dolor a la palpación.";
    } else {
      textoGenerado = `Cuello: ${cuello}`;
    }
            ...
            {showRedaccion ? (
              ...
              <div className="p-6 space-y-6">
                <div className="space-y-2">
                  <Label htmlFor="cuello">Describa la exploración del cuello:</Label>
                  <Textarea
                    id="cuello"
                    placeholder="Forma, adenomegalias, masas, cicatrices, movilidad, etc..."
                  ...
                  <div className="mt-2">
                    <VoiceInput
                      onTranscriptionComplete={(text) => {
                        const currentText = formData.cuello || "";
                        handleInputChange("cuello", currentText ? `${currentText} ${text}` : text);
                      }}
                    />
                  </div>
                </div>
