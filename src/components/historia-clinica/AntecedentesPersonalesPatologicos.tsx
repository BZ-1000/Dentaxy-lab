...
const AntecedentesPersonalesPatologicos: React.FC<AntecedentesPersonalesPatologicosProps> = ({
  ...
  const PatologiaToggle = ({ checked, onChange }: { checked: boolean, onChange: (checked: boolean) => void }) => {
      ...
      >
        ...
        {checked && (
          <p className="text-xs text-blue-600 dark:text-blue-400 mt-2 ml-7">
            Si el paciente no presenta patologías, no es necesario rellenar las secciones.
          </p>
        )}
      </div>
    );
