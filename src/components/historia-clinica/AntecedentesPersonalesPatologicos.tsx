...
import { Minus, Maximize2, X, Eraser, Copy, CheckCircle, AlertCircle, EyeOff, Eye } from "lucide-react";
...
const AntecedentesPersonalesPatologicos: React.FC<AntecedentesPersonalesPatologicosProps> = ({
  ...
  const handleSinPatologiaChange = () => {
    ...
    if (newValue) {
      ...
      limpiarFormulario();
    } else if (previousFormState) {
      // Restaurar el estado anterior del formulario al volver a mostrar las secciones
        ...
        {!isMinimized && <div className="p-6" ref={formRef}>
          {showForm ? (
            <div className="space-y-6">
              ...
              <div
                className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-100 dark:border-blue-800 w-full text-left cursor-pointer transition-all hover:bg-blue-100 dark:hover:bg-blue-900/30"
              ...
              >
                ...
                {sinPatologia && (
                  <p className="text-xs text-blue-600 dark:text-blue-400 mt-2 ml-7">
                    El paciente no presenta patologías. Presione nuevamente para mostrar el formulario completo.
                  </p>
                )}
              </div>
