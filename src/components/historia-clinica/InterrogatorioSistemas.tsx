...
function InterrogatorioSistemas() {
  ...
  // Aquí suponemos que existe una función para generar la redacción del aparato digestivo, la vamos a actualizar:
function generarRedaccionAparatoDigestivo(datos) {
  // datos contiene las variables de la forma: tipoAlimentacion, patronMasticacion, percepcionGusto, salivacion, dificultadDeglucion, halitosis, sintomasDigestivos, frecuenciaEvacuacion
  ...
  // Verificamos si todos los síntomas digestivos están en "ninguno"
  const todosNinguno = sintomasDigestivos.length === 0 || sintomasDigestivos.every(sintoma => sintoma.toLowerCase() === "ninguno");
  ...
  if (!todosNinguno) {
    // Listar síntomas distintos de ningunos separados por comas, si hay
    const sintomasValidos = sintomasDigestivos.filter(s => s.toLowerCase() !== "ninguno");
