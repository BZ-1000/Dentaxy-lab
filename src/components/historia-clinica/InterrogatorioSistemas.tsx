...
const InterrogatorioSistemas = ({ formData, handleInterrogatorioChange }: InterrogatorioSistemasProps) => {
  ...
  const generateRedaccion = () => {
    try {
      ...
      // Mapear valores a texto para la redacción
      ...
      // Generar texto de la redacción
      let interrogatorioText = "Interrogatorio por sistemas:\n\n";
      
      // Sistema Respiratorio
      interrogatorioText += "Sistema Respiratorio: ";
      const respiratorio = [];
      if (formData.interrogatorioSistemas.tosExpectoracion) {
        respiratorio.push(tosExpectoracionText);
      }
      if (formData.interrogatorioSistemas.disnea) {
        respiratorio.push(`Disnea ${formData.interrogatorioSistemas.disnea}.`);
      }
      if (formData.interrogatorioSistemas.sibilancias === 'si') {
        respiratorio.push("Presenta sibilancias.");
      }
      if (formData.interrogatorioSistemas.dolorToracico === 'si') {
        respiratorio.push("Refiere dolor torácico.");
      }
      interrogatorioText += respiratorio.length > 0 ? respiratorio.join(" ") : "Sin hallazgos significativos.";
      interrogatorioText += "\n\n";
      
      // Sistema Cardiovascular
      interrogatorioText += "Sistema Cardiovascular: ";
      const cardiovascular = [];
      if (formData.interrogatorioSistemas.palpitaciones === 'si') {
        cardiovascular.push("Refiere palpitaciones.");
      }
      if (formData.interrogatorioSistemas.edema === 'si') {
        cardiovascular.push("Presenta edema.");
      }
      if (formData.interrogatorioSistemas.sincope === 'si') {
        cardiovascular.push("Ha presentado síncope.");
      }
      interrogatorioText += cardiovascular.length > 0 ? cardiovascular.join(" ") : "Sin hallazgos significativos.";
      interrogatorioText += "\n\n";
      
      // Sistema Digestivo
      interrogatorioText += "Sistema Digestivo: ";
      const digestivo = [];
      if (formData.interrogatorioSistemas.nauseas === 'si') {
        digestivo.push("Refiere náuseas.");
      }
      if (formData.interrogatorioSistemas.vomitos === 'si') {
        digestivo.push("Presenta vómitos.");
      }
      if (formData.interrogatorioSistemas.diarrea === 'si') {
        digestivo.push("Presenta diarrea.");
      }
      if (formData.interrogatorioSistemas.estrenimiento === 'si') {
        digestivo.push("Refiere estreñimiento.");
      }
      interrogatorioText += digestivo.length > 0 ? digestivo.join(" ") : "Sin hallazgos significativos.";
      interrogatorioText += "\n\n";
      
      // Sistema Urinario
      interrogatorioText += "Sistema Urinario: ";
      const urinario = [];
      if (formData.interrogatorioSistemas.disuria === 'si') {
        urinario.push("Presenta disuria.");
      }
      if (formData.interrogatorioSistemas.polaquiuria === 'si') {
        urinario.push("Refiere polaquiuria.");
      }
      if (formData.interrogatorioSistemas.urgenciaMiccional === 'si') {
        urinario.push("Presenta urgencia miccional.");
      }
      if (formData.interrogatorioSistemas.nicturia === 'si') {
        urinario.push("Presenta nicturia.");
      }
      interrogatorioText += urinario.length > 0 ? urinario.join(" ") : "Sin hallazgos significativos.";
      interrogatorioText += "\n\n";
      
      // Sistema Músculo-esquelético
      interrogatorioText += "Sistema Músculo-esquelético: ";
      const musculoEsqueletico = [];
      if (formData.interrogatorioSistemas.artralgias === 'si') {
        musculoEsqueletico.push("Presenta artralgias.");
      }
      if (formData.interrogatorioSistemas.mialgias === 'si') {
        musculoEsqueletico.push("Refiere mialgias.");
      }
      if (formData.interrogatorioSistemas.rigidezMatutina) {
        musculoEsqueletico.push(rigidezMatutinaText);
      }
      interrogatorioText += musculoEsqueletico.length > 0 ? musculoEsqueletico.join(" ") : "Sin hallazgos significativos.";
      interrogatorioText += "\n\n";
      
      // Sistema Nervioso
      interrogatorioText += "Sistema Nervioso: ";
      const nervioso = [];
      if (formData.interrogatorioSistemas.cefalea === 'si') {
        nervioso.push("Refiere cefalea.");
      }
      if (formData.interrogatorioSistemas.mareos === 'si') {
        nervioso.push("Presenta mareos.");
      }
      if (formData.interrogatorioSistemas.parestesias === 'si') {
        nervioso.push("Refiere parestesias.");
      }
      interrogatorioText += nervioso.length > 0 ? nervioso.join(" ") : "Sin hallazgos significativos.";
      interrogatorioText += "\n\n";
      
      // Sistema Endocrino
      interrogatorioText += "Sistema Endocrino: ";
      const endocrino = [];
      if (formData.interrogatorioSistemas.polidipsia === 'si') {
        endocrino.push("Presenta polidipsia.");
      }
      if (formData.interrogatorioSistemas.poliuria === 'si') {
        endocrino.push("Refiere poliuria.");
      }
      if (formData.interrogatorioSistemas.polifagia === 'si') {
        endocrino.push("Presenta polifagia.");
      }
      if (formData.interrogatorioSistemas.cambiosMenstruales) {
        endocrino.push(cambiosMenstrualesText);
      }
      interrogatorioText += endocrino.length > 0 ? endocrino.join(" ") : "Sin hallazgos significativos.";
      interrogatorioText += "\n\n";
      
      // Sistema Tegumentario
      interrogatorioText += "Sistema Tegumentario: ";
      const tegumentario = [];
      if (formData.interrogatorioSistemas.cambiosPiel === 'si') {
        tegumentario.push("Refiere cambios en la piel.");
      }
      if (formData.interrogatorioSistemas.cambiosCabello === 'si') {
        tegumentario.push("Presenta cambios en el cabello.");
      }
      if (formData.interrogatorioSistemas.cambiosUnas) {
        tegumentario.push(cambiosUnasText);
      }
      interrogatorioText += tegumentario.length > 0 ? tegumentario.join(" ") : "Sin hallazgos significativos.";
      interrogatorioText += "\n\n";
      
      // Otros
      interrogatorioText += "Otros: ";
      const otros = [];
      if (formData.interrogatorioSistemas.fiebre === 'si') {
        otros.push("Presenta fiebre.");
      }
      if (formData.interrogatorioSistemas.perdidaPeso === 'si') {
        otros.push("Refiere pérdida de peso.");
      }
      
      if (formData.interrogatorioSistemas.habitosAlimenticios) {
        otros.push(`Hábitos alimenticios: ${habitosAlimenticiosText}`);
      }
      
      interrogatorioText += otros.length > 0 ? otros.join(" ") : "Sin hallazgos significativos.";
      
      // Actualizar el estado de redacción
      setRedaccion({ interrogatorio: interrogatorioText });
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="formulario">
              ...
              <div className="space-y-2">
                ...
                >
                  <div className="flex items-center space-x-2">
                    ...
                    <Label htmlFor="habitosAlimenticios-ninguno">Ninguno relevante</Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="ingesta nocturna" id="habitosAlimenticios-nocturna" />
                    <Label htmlFor="habitosAlimenticios-nocturna">Ingesta nocturna</Label>
                  </div>
                  <div className="flex items-center space-x-2">
          ...
          <div className="p-6 flex justify-end">
            <Button onClick={generateRedaccion} disabled={isGenerating}>
              {isGenerating ? (
                <>
        ...
        <TabsContent value="redaccion">
          <CardContent>
            <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded-md min-h-[200px] whitespace-pre-wrap" data-redaction-content>
              {redaccion.interrogatorio ? (
                <AnimatedTextareaWithTyping 
                  content={redaccion.interrogatorio}
                  disabled
                  className="w-full bg-transparent border-none focus:ring-0 resize-none text-sm" 
                />
              ) : (
                <div className="text-center text-gray-500 py-8">
                  No hay redacción generada. Por favor, complete el formulario y genere la redacción.
                </div>
              )}
            </div>
          </CardContent>
