...
interface AntecedentesMedicosQuirurgicosProps {
  formData: FormDataState;
  handleInputChange: (field: string, value: string) => void;
}

const AntecedentesMedicosQuirurgicos = ({ formData, handleInputChange }: AntecedentesMedicosQuirurgicosProps) => {
  ...
  const generarRedaccionIA = () => {
    const antecedentesMedicos = formData.antecedentesMedicos || "";
    
    let textoGenerado = "";
    if (!antecedentesMedicos || antecedentesMedicos.trim() === "") {
      textoGenerado = "El paciente niega antecedentes médicos o quirúrgicos de relevancia para su condición actual.";
    } else {
      textoGenerado = `El paciente reporta los siguientes antecedentes médicos y quirúrgicos: ${antecedentesMedicos}`;
    }
  ...
  const limpiarFormulario = () => {
    handleInputChange("antecedentesMedicos", "");
    setRedaccionIA("");
            ...
            {showRedaccion ? (
              ...
              <div className="p-6 space-y-6">
                <div className="space-y-2">
                  <Label htmlFor="antecedentesMedicos">Describa los antecedentes médicos y quirúrgicos del paciente:</Label>
                  <Textarea
                    id="antecedentesMedicos"
                    placeholder="Tratamientos médicos actuales, enfermedades diagnosticadas, procedimientos quirúrgicos previos, etc..."
                  ...
                  <div className="mt-2">
                    <VoiceInput
                      onTranscriptionComplete={(text) => {
                        const currentText = formData.antecedentesMedicos || "";
                        handleInputChange("antecedentesMedicos", currentText ? `${currentText} ${text}` : text);
                      }}
                    />
                  </div>
                </div>
...
export default AntecedentesMedicosQuirurgicos;
