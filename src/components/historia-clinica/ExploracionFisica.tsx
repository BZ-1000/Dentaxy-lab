...
const ExploracionFisica = ({ formData, handleInputChange }: ExploracionFisicaProps) => {
  ...
  const generarRedaccionIA = () => {
    const exploracionFisica = formData.exploracionFisica || "";
    
    let textoGenerado = "";
    if (!exploracionFisica || exploracionFisica.trim() === "") {
      textoGenerado = "A la exploración física el paciente se encuentra en buen estado general, consciente, orientado, con marcha normal y adecuada coloración de piel y mucosas.";
    } else {
      textoGenerado = `A la exploración física: ${exploracionFisica}`;
    }
  ...
  const limpiarFormulario = () => {
    handleInputChange("exploracionFisica", "");
    setRedaccionIA("");
            ...
            {showRedaccion ? (
              ...
              <div className="p-6 space-y-6">
                <div className="space-y-2">
                  <Label htmlFor="exploracionFisica">Describa los hallazgos de la exploración física general:</Label>
                  <Textarea
                    id="exploracionFisica"
                    placeholder="Estado general, consciencia, marcha, características de piel y mucosas, etc..."
                    value={formData.exploracionFisica || ""}
                    onChange={(e) => handleInputChange("exploracionFisica", e.target.value)}
                  ...
                  <div className="mt-2">
                    <VoiceInput
                      onTranscriptionComplete={(text) => {
                        const currentText = formData.exploracionFisica || "";
                        handleInputChange("exploracionFisica", currentText ? `${currentText} ${text}` : text);
                      }}
                    />
                  </div>
                </div>
