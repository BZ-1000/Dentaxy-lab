...
interface ExamenDeCabezaProps {
  formData: FormDataState;
  handleInputChange: (field: string, value: string) => void;
}

const ExamenDeCabeza = ({ formData, handleInputChange }: ExamenDeCabezaProps) => {
  ...
  const generarRedaccionIA = () => {
    const examenCabeza = formData.examenCabeza || "";
    
    let textoGenerado = "";
    if (!examenCabeza || examenCabeza.trim() === "") {
      textoGenerado = "A la exploración de cabeza, se observa una configuración normocéfalo, sin asimetrías o lesiones visibles.";
    } else {
      textoGenerado = `Examen de cabeza: ${examenCabeza}`;
    }
  ...
  const limpiarFormulario = () => {
    handleInputChange("examenCabeza", "");
    setRedaccionIA("");
            ...
            {showRedaccion ? (
              ...
              <div className="p-6 space-y-6">
                <div className="space-y-2">
                  <Label htmlFor="examenCabeza">Describa el examen de cabeza:</Label>
                  <Textarea
                    id="examenCabeza"
                    placeholder="Configuración, simetría, lesiones visibles, etc..."
                    value={formData.examenCabeza || ""}
                    onChange={(e) => handleInputChange("examenCabeza", e.target.value)}
                  ...
                  <div className="mt-2">
                    <VoiceInput
                      onTranscriptionComplete={(text) => {
                        const currentText = formData.examenCabeza || "";
                        handleInputChange("examenCabeza", currentText ? `${currentText} ${text}` : text);
                      }}
                    />
                  </div>
                </div>
