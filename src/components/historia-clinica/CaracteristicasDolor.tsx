...
const localizacionesEjemplo = ["la región molar inferior izquierda...", "la articulación temporomandibular izquierda...", "el piso de boca, irradiado hacia la lengua...", "la papila interdentaria entre los incisivos inferiores..."];
...
const CaracteristicasDolor = ({
  dolor,
  onDolorChange
}: CaracteristicasDolorProps) => {
  ...
  return <div className="space-y-4">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline" size="icon" className="relative h-10 w-10 rounded-full border-2 border-primary/20 transition-all duration-100 shadow-md hover:shadow-lg font-normal text-neutral-50 bg-sky-500 hover:bg-sky-400">
              <div className="relative">
                ...
                {showIcon && <div className="absolute -bottom-3 -right-3 animate-bounce">
          ...
          <PopoverContent className="w-80 p-4 bg-white/95 backdrop-blur-sm shadow-xl border border-primary/10 rounded-lg">
            <div className="space-y-4">
              {definicionesDolor.map((def, index) => <div key={index} className="space-y-2 p-3 rounded-lg hover:bg-accent/10 transition-colors duration-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input type="date" value={dolor.fechaInicio} onChange={e => onDolorChange('fechaInicio', e.target.value)} className="w-full px-4 py-2 rounded-lg border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all duration-200 bg-white/50 backdrop-blur-sm shadow-sm hover:shadow-md" />
        </div>
      ...
      <div>
        ...
        <div className="flex items-start gap-4">
          <div className="relative w-full">
            <Textarea value={localizacionText} onChange={handleLocalizacionChange} onKeyDown={e => handleKeyDown(defaultLocalizacion, e)} placeholder={defaultLocalizacion} className="min-h-[100px] max-h-[200px] w-full resize-y bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-md text-justify" />
            {localizacionText === defaultLocalizacion && <div className="absolute top-2 left-[115px] pointer-events-none">
                <Typewriter text={localizacionesEjemplo} speed={50} deleteSpeed={30} delay={2000} loop={true} className="text-gray-500 italic text-base" />
              </div>}
          </div>
          <div className="mt-2">
      ...
      <div>
        ...
        <div className="flex items-start gap-4">
          <Textarea value={dolor.atenuacion} onChange={e => onDolorChange('atenuacion', e.target.value)} placeholder="Describe que empeora el dolor (frío, caliente,) o que lo disminuye (analgésicos)" className="min-h-[100px] max-h-[200px] w-full resize-y text-justify" />
          <div className="mt-2">
      ...
      {dolor.condicionAparicion === 'provocado' && <div className="mt-4">
          <Label className="text-gray-700 dark:text-gray-300">Causa del dolor provocado:</Label>
          <div className="flex items-start gap-4">
            <div className="relative w-full">
              <Textarea value={causaProvocadoText} onChange={handleCausaProvocadoChange} onKeyDown={e => handleKeyDown(defaultCausaProvocado, e)} placeholder={defaultCausaProvocado} className="min-h-[100px] max-h-[200px] w-full resize-y bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-md text-justify" />
              {causaProvocadoText === defaultCausaProvocado && <div className="absolute top-2 left-[115px] pointer-events-none">
                  <Typewriter text={causasProvocadoEjemplo} speed={50} deleteSpeed={30} delay={2000} loop={true} className="text-gray-500 italic text-base" />
                </div>}
            </div>
            <div className="mt-2">
              <VoiceInput onTranscriptionComplete={text => {
