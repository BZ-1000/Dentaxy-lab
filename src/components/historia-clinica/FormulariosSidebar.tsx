...
const FormulariosSidebar = ({
  ...
  return <div className="">
      <div className="sticky top-0 h-screen hidden md:block">
        <div className={`h-full bg-white dark:bg-neutral-900 border-r border-neutral-200 dark:border-neutral-800 transition-all duration-300 ${open ? 'w-64' : 'w-16'}`}>
          <div className="p-4">
            <div className="flex items-center justify-between mb-6">
              {open ? (
                <div className="flex items-center gap-2">
                  <BookOpen className="flex-shrink-0" size={24} color={theme === 'dark' ? 'white' : '#3b82f6'} />
                  <span className="font-semibold text-lg">Formularios</span>
                </div>
              ) : (
                <BookOpen className="flex-shrink-0" size={24} color={theme === 'dark' ? 'white' : '#3b82f6'} />
              )}
            ...
            <ScrollArea className="flex-1">
              <div className="space-y-1 pr-2">
                {formularios.map((form, index) => (
                  <div key={index} className="group flex justify-between items-center mb-2 px-2">
                    <button
                      onClick={() => onCargarFormulario(form.data, form.nombre)}
                      className="flex items-center gap-2 hover:bg-neutral-200 dark:hover:bg-neutral-700 rounded-md px-2 py-2 flex-1 text-left"
                    >
