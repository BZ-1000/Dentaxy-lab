...
interface InterrogatorioPorAparatosYSistemasProps {
  formData: FormDataState;
  handleInputChange: (field: string, value: string) => void;
}

const InterrogatorioPorAparatosYSistemas = ({ formData, handleInputChange }: InterrogatorioPorAparatosYSistemasProps) => {
  ...
  const generarRedaccionIA = () => {
    ...
    let textoGenerado = "";
    if (!interrogatorio || interrogatorio.trim() === "") {
      textoGenerado = "Al interrogatorio por aparatos y sistemas, el paciente no refiere alteraciones significativas.";
    } else {
      textoGenerado = `Al interrogatorio por aparatos y sistemas, el paciente refiere: ${interrogatorio}`;
    }
  ...
  const limpiarFormulario = () => {
    handleInputChange("interrogatorioSistemas", "");
    setRedaccionIA("");
            ...
            {showRedaccion ? (
              ...
              <div className="p-6 space-y-6">
                <div className="space-y-2">
                  <Label htmlFor="interrogatorioSistemas">Interrogatorio por Aparatos y Sistemas:</Label>
                  <Textarea
                    id="interrogatorioSistemas"
                    placeholder="Describa cualquier hallazgo relevante en los diferentes aparatos y sistemas (respiratorio, cardiovascular, digestivo, etc...)"
                    value={formData.interrogatorioSistemas || ""}
                    onChange={(e) => handleInputChange("interrogatorioSistemas", e.target.value)}
                    className="min-h-[200px] max-h-[400px] w-full resize-y bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-md"
                  />
                  <div className="mt-2">
                    <VoiceInput
                      onTranscriptionComplete={(text) => {
                        const currentText = formData.interrogatorioSistemas || "";
                        handleInputChange("interrogatorioSistemas", currentText ? `${currentText} ${text}` : text);
                      }}
                    />
                  </div>
                </div>
...
export default InterrogatorioPorAparatosYSistemas;
