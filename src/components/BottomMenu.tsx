...
const BottomMenu = () => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg border-t border-gray-200 dark:border-gray-800 shadow-lg z-50">
      <nav className="max-w-screen-xl mx-auto px-1 md:px-4">
        <div className="flex justify-around items-center h-12 md:h-16 overflow-x-auto">
          {menuItems.map((item) => (
            <Link 
              key={item.path}
              to={item.path} 
              className={`flex flex-col items-center gap-0.5 px-0.5 md:px-1 ${
            ...
            >
              ...
              <span className="text-[0.6rem] md:text-xs">{item.label}</span>
            </Link>
          ))}
          
          <button 
            onClick={handleSettingsClick}
            className="flex flex-col items-center gap-0.5 px-0.5 md:px-1 text-gray-600 hover:text-primary dark:text-gray-400 dark:hover:text-primary transition-colors duration-200"
          >
            <Settings className="h-3.5 w-3.5 md:h-5 md:w-5" />
            <span className="text-[0.6rem] md:text-xs">Ajustes</span>
          </button>
          ...
          >
            ...
            <span className="text-[0.6rem] md:text-xs">Perfil</span>
          </button>
        </div>
      </nav>
    </div>
