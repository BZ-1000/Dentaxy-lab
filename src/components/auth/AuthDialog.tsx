...
export function AuthDialog({
  ...
  return <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-full max-w-md rounded-lg border border-gray-100 bg-white p-0 shadow-lg">
        <div className="h-2 w-full rounded-full bg-slate-50"></div>
        <div className="p-6">
          <div className="text-center">
            <h2 className="text-2xl font-bold text-gray-800">
              {mode === "login" ? "Iniciar Sesión" : "Crear Cuenta"}
            </h2>
            {mode === "register" && <p className="mt-2 text-sm text-gray-600">
                Únete al equipo Dental Basics Academy IA
              </p>}
          </div>

          <div className="mt-6">
            <Button type="button" onClick={handleGoogleLogin} className="w-full flex items-center justify-center gap-2 bg-white hover:bg-gray-50 text-gray-700 border border-gray-300 h-12" variant="outline">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-4">
              <div>
                ...
                <Input id="email" type="email" value={email} onChange={e => setEmail(e.target.value)} className="mt-1 h-10 border-gray-300 focus:border-blue-500 focus:ring-blue-500" placeholder="tu@email.com" required />
              </div>
              ...
              <div>
                ...
                <Input id="password" type="password" value={password} onChange={e => setPassword(e.target.value)} className="mt-1 h-10 border-gray-300 focus:border-blue-500 focus:ring-blue-500" required />
              </div>
            </div>
          ...
          <p className="mt-6 text-center text-sm text-gray-600">
            ...
            <button onClick={() => setMode(mode === "login" ? "register" : "login")} className="font-medium text-blue-600 hover:text-blue-800" type="button">
