...
import {
  FlaskConical, ArrowLeft, ArrowRight, Sparkles,
  FileDown, Printer, Scan, MessageSquare, CheckCircle,
  Lock, ShieldCheck, Wifi, Layers, Zap, Clock, Upload,
  PackageCheck, Settings, ChevronRight, X, Mail, User, Loader2,
} from "lucide-react";
...
export default function LabLogin() {
  ...
  const [waitlistCount] = useState(219);

  return (
    <div className="lab-theme min-h-screen w-full relative bg-white overflow-hidden font-sans">
      <WaitlistMasterModal isOpen={waitlistOpen} onClose={() => setWaitlistOpen(false)} preselectedModule="Lab" />
      <OrganicShopFrame
      ...
      <main className="relative z-10 min-h-screen w-full flex flex-col items-center justify-center p-6 pt-24">
        ...
        >
          ...
          <motion.h1 variants={fadeUp} className="text-5xl md:text-7xl lg:text-8xl font-bold tracking-tight">
            ...
            <span className="eco-gradient-text">Lab</span>
          </motion.h1>

          {/* Tagline */}
          <motion.p variants={fadeUp} className="text-xl md:text-2xl eco-accent-text font-medium">
            El puente digital con tu laboratorio
          </motion.p>

          {/* Description */}
          <motion.p variants={fadeUp} className="text-lg text-gray-500 max-w-2xl mx-auto">
            Gestión de trabajos protésicos, envío de archivos 3D y comunicación directa con laboratorios — sin llamadas, sin papel, sin intermediarios.
          </motion.p>
          ...
          <motion.div variants={staggerContainer} className="flex flex-col sm:flex-row gap-6 w-full max-w-2xl">
            <motion.button
              ...
              className="flex-1 group relative bg-white border border-neutral-100 p-6 rounded-3xl text-left shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_-12px_rgba(124,58,237,0.2)] transition-all duration-500 hover:-translate-y-1"
            >
              <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
                <ArrowRight className="w-4 h-4 text-purple-500 -rotate-45" />
              </div>
              <div className="bg-purple-50 w-14 h-14 rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-500">
                <Mail className="w-7 h-7 text-purple-600" />
              </div>
              <h3 className="font-bold text-neutral-900 text-xl group-hover:text-purple-700 transition-colors">Lista de Espera</h3>
            ...
            <motion.button
              ...
              className="flex-1 group relative bg-white border border-neutral-100 p-6 rounded-3xl text-left shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_-12px_rgba(124,58,237,0.15)] transition-all duration-500 hover:-translate-y-1"
            >
              <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            className="grid sm:grid-cols-3 gap-4 w-full max-w-3xl mt-4"
          >
            {[
              { icon: Scan, label: "Envío de Archivos 3D", color: "text-purple-500", bg: "bg-purple-50" },
              { icon: MessageSquare, label: "Chat directo Lab", color: "text-violet-500", bg: "bg-violet-50" },
              { icon: PackageCheck, label: "Seguimiento de Trabajos", color: "text-fuchsia-500", bg: "bg-fuchsia-50" },
            ].map((f, i) => (
      ...
      {openModal !== "none" && (
        ...
        >
          ...
          >
            <button onClick={() => setOpenModal("none")} className="absolute top-6 right-6 w-8 h-8 flex items-center justify-center rounded-full bg-neutral-100 hover:bg-neutral-200 transition-colors">
            ...
            {openModal === "admin" && (
              <div className="space-y-6 text-center">
                ...
                <h2 className="text-2xl font-bold">Admin Lab</h2>
