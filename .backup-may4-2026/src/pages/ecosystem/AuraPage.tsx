...
import {
  Award, ArrowRight, Sparkles, Medal, GraduationCap,
  Image, Star, Lock, X, Mail, User, Loader2, CheckCircle, BadgeCheck,
} from "lucide-react";
...
export default function AuraPage() {
  ...
  const [waitlistCount] = useState(384);

  return (
    <div className="aura-theme min-h-screen w-full relative bg-white overflow-hidden font-sans">
      <WaitlistMasterModal isOpen={waitlistOpen} onClose={() => setWaitlistOpen(false)} preselectedModule="Aura" />
      <OrganicShopFrame
      ...
      <main className="relative z-10 min-h-screen w-full flex flex-col items-center justify-center p-6 pt-24">
        ...
        >
          ...
          <motion.h1 variants={fadeUp} className="text-5xl md:text-7xl lg:text-8xl font-bold tracking-tight">
            ...
            <span className="eco-gradient-text">Aura</span>
          </motion.h1>

          <motion.p variants={fadeUp} className="text-xl md:text-2xl eco-accent-text font-medium">
            Tu portafolio de prestigio profesional
          </motion.p>

          <motion.p variants={fadeUp} className="text-lg text-gray-500 max-w-2xl mx-auto">
            El altar digital donde exhibes tus títulos, certificaciones, especialidades y casos de éxito. Tu reputación clínica, visible ante el mundo con la credibilidad que merece.
          </motion.p>
          ...
          <motion.div variants={staggerContainer} className="flex flex-col sm:flex-row gap-6 w-full max-w-2xl">
            <motion.button variants={scaleIn} onClick={() => setWaitlistOpen(true)}
              className="flex-1 group relative bg-white border border-neutral-100 p-6 rounded-3xl text-left shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_-12px_rgba(245,158,11,0.25)] transition-all duration-500 hover:-translate-y-1"
            >
              <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
                <ArrowRight className="w-4 h-4 text-amber-500 -rotate-45" />
              </div>
              <div className="bg-amber-50 w-14 h-14 rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-500">
                <Award className="w-7 h-7 text-amber-600" />
              </div>
              <h3 className="font-bold text-neutral-900 text-xl group-hover:text-amber-700 transition-colors">Lista de Espera</h3>
              <p className="text-sm text-neutral-500">Construir mi perfil de prestigio</p>
            </motion.button>

            <motion.button variants={scaleIn} onClick={() => navigate("/hub")}
              className="flex-1 group relative bg-white border border-neutral-100 p-6 rounded-3xl text-left shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_-12px_rgba(245,158,11,0.1)] transition-all duration-500 hover:-translate-y-1"
            >
          ...
          >
            {[
              { icon: BadgeCheck, label: "Certificaciones Verificadas", color: "text-amber-500", bg: "bg-amber-50" },
              { icon: Image, label: "Galería de Casos Clínicos", color: "text-yellow-600", bg: "bg-yellow-50" },
              { icon: GraduationCap, label: "Historial Académico", color: "text-orange-500", bg: "bg-orange-50" },
            ].map((f, i) => (
          ...
          >
            ...
            {openModal === "admin" && (
              <div className="space-y-4 text-center">
                ...
                <h2 className="text-2xl font-bold">Admin Aura</h2>
